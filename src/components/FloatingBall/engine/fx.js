...
(function (global) {
  ...
  const Re0 = 114.2705;
  const STAR_COLOR = "#a6a099";
  // 电子声带演示定制：粒子/彩带用黑白灰区间（与墨色球体风格一致），原彩色板见 grok 仓库
  const PALETTE = ["#1a1816", "#4a4642", "#7a756e", "#a6a099", "#d8d3cc", "#ffffff"];
  const STAR = (() => {
    const n = [];
    for (let e = 0; e < 10; e++) {
          ...
          if (Ae.length >= 2) {
            ...
            if (j.stops) {
              ...
              for (let we = 0; we < j.stops.length; we++) {
                ...
                // 电子声带演示定制：彩带改灰阶（原为 hue 彩色渐变）
                  j.stops[we].setAttribute("stop-color", `hsl(0 0% ${(30 + 45 * Pe).toFixed(0)}%)`);
              }
            }
