...
pub fn load_settings(data_dir: &Path) -> Settings {
    ...
    let mut settings = match serde_json::from_str::<serde_json::Value>(&raw) {
        Ok(v) => Settings {
            ...
            subtitle_enabled: v.get("subtitle_enabled").and_then(|x| x.as_bool()).unwrap_or(default.subtitle_enabled),
            subtitle_always_on_top: v.get("subtitle_always_on_top").and_then(|x| x.as_bool()).unwrap_or(default.subtitle_always_on_top),
            subtitle_opacity: v.get("subtitle_opacity").and_then(|x| x.as_f64()).map(|x| x as f32).unwrap_or(default.subtitle_opacity),
            subtitle_font_size: v.get("subtitle_font_size").and_then(|x| x.as_u64()).map(|x| x as u32).unwrap_or(default.subtitle_font_size),
            subtitle_position: v.get("subtitle_position").and_then(|x| x.as_str()).map(String::from).unwrap_or(default.subtitle_position),
            subtitle_target_process: v.get("subtitle_target_process").and_then(|x| x.as_str()).map(String::from).unwrap_or(default.subtitle_target_process),
            subtitle_asr_plugin: v.get("subtitle_asr_plugin").and_then(|x| x.as_str()).map(String::from).unwrap_or(default.subtitle_asr_plugin),
            subtitle_language: v.get("subtitle_language").and_then(|x| x.as_str()).map(String::from).unwrap_or(default.subtitle_language),
            subtitle_vad_sensitivity: v.get("subtitle_vad_sensitivity").and_then(|x| x.as_str()).map(String::from).unwrap_or(default.subtitle_vad_sensitivity),
