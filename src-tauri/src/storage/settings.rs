...
pub fn load_settings(data_dir: &Path) -> Settings {
    ...
    match serde_json::from_str::<serde_json::Value>(&raw) {
        Ok(v) => Settings {
            ...
            edge_voice: v.get("edge_voice").and_then(|x| x.as_str()).map(String::from).unwrap_or(default.edge_voice),
...
pub fn update_setting(data_dir: &Path, key: &str, value: serde_json::Value) -> Result<Settings> {
    ...
    match key {
        ...
        "edge_voice" => if let Some(v) = value.as_str() { s.edge_voice = v.to_string() },
