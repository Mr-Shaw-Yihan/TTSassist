...
# 注意：hf-mirror 各节点行为不一致（缓存未命中时可能 308 回源 huggingface.co），
# 因此所有下载走 snapshot_download_with_fallback 多端点回退，不依赖单一源。
...
def endpoint_candidates() -> list:
    """下载端点候选：配置端点 → hf-mirror → 官方源（去重）"""
    cands = []
    for c in (CONFIG.get("hf_endpoint", ""), "https://hf-mirror.com", "https://huggingface.co"):
        c = (c or "").strip().rstrip("/")
...
def download_predefined(voice_id: str) -> str:
    """从 HF 下载预置角色到 characters/<voice_id>/，并写 meta.json（多端点回退）"""
