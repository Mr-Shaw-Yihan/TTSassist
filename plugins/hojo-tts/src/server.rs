// Hojo 服务子进程管理：拉起 hojo_server.py、健康探活、崩溃后重启。
//
...
pub fn ensure_server(ctx: &Ctx) -> Result<u16, String> {
    ...
    cmd.arg(ctx.server_script())
        ...
        // 强制 UTF-8：日志含中文，GBK 控制台会乱码/崩溃
        .env("PYTHONIOENCODING", "utf-8")
        .env("PYTHONUTF8", "1")
        // HF 端点配置透传（宿主通用插件配置注入的环境变量 → 服务端下载用）
        .env(crate::paths::ENV_HF_ENDPOINT, crate::paths::hf_endpoint())
    ...
    let child = cmd
        .spawn()
        .map_err(|e| format!("启动 Hojo 服务失败: {e}"))?;
    ...
    // 等待 /health 就绪（服务端启动只导入 fastapi，不碰 torch，通常几秒内）
