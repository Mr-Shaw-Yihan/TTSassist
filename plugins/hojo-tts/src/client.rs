// HTTP 客户端：两部分——
// 1. 引导期文件下载（Python 运行时、预置音色参考音频，多源回退直下到磁盘）
// 2. 与 hojo_server.py 的 API 通信（health / ensure_models / load_voice / tts）
...
fn http() -> Result<reqwest::blocking::Client, String> {
    reqwest::blocking::Client::builder()
        .user_agent("VoiceAssist-HojoPlugin")
...
/// 多源回退下载：按顺序尝试各 URL，任一成功即返回；全部失败才报错
pub fn download_with_fallback(
    ...
    for url in urls {
        match download_file_with_progress(url, dest, progress) {
            Ok(()) => return Ok(()),
            Err(e) => {
                last_err = e;
    ...
    Err(format!("全部下载源均失败，最后错误: {last_err}"))
}
...
/// 确保 ONNX 模型就绪（首次约 460MB，服务端幂等下载 + 多端点回退）
pub fn ensure_models(port: u16) -> Result<(), String> {
    let client = api_client(3600)?;
    let resp = client
        .post(format!("http://127.0.0.1:{port}/ensure_models"))
        .json(&serde_json::json!({}))
        .send()
        .map_err(|e| format!("模型准备请求失败: {e}"))?;
    if resp.status().is_success() {
        return Ok(());
    }
    let detail = error_detail(resp);
    Err(format!(
        "Hojo 模型下载失败（首次约 460MB，请保持联网后重试）: {detail}"
    ))
}

/// 加载音色（校验音色包 + 构造推理引擎预热；首次含模型加载，耗时给足）
pub fn load_voice(port: u16, voice_id: &str) -> Result<(), String> {
    let client = api_client(1800)?;
    let resp = client
        .post(format!("http://127.0.0.1:{port}/load_voice"))
...
/// 合成：文本 → 完整 WAV 字节（24kHz 单声道 16bit）
